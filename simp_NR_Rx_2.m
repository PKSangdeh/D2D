...
phase_track_type=1;   % 1 for regular and 2 for polynomial fit
samp_shift=-0;
%% Frame Parameters
cp_length=38;
num_str=2;
Nz=300;
n_ant=4;                        % Number of BS's antenna
...
for st=1:num_str
    ...
    for ii = 1:n_ant
        file_id = fopen(['signals/nr_rx_' num2str(ii-1) '.dat'], 'rb');
        if (file_id < 0)
    ...
    %% fecth one frame
    num_sample_processing=2*num_sample_processing;
    rx_signal_buf = zeros(n_ant, num_sample_processing);
    for ii = 1:n_ant
        file_id = fopen(['signals/nr_rx_' num2str(ii-1) '.dat'], 'rb');
        if (file_id < 0)
...
%% EVM
for i=1:num_str
    disp(['EVM of USER ',num2str(i)])
    evm_cel(i,:)
end
%tmp1=tmp1/norm(tmp1);
% h1=[1+1j;1.2-1j;3+1.3j;0.7-0.2j];
% h2=[0.5+1.4j;1.4-1.1j;5.5-0.1j;2+1.2j];
% v1=squeeze(G_stack(1,1,:));
% v2=squeeze(G_stack(1,2,:));
% v3=squeeze(G_stack(1,3,:));
% v4=squeeze(G_stack(1,4,:));
% tmp1
