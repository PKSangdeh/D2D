...
num_str=3;
Nz=300;
n_ant=1;                        % Number of BS's antenna
...
for st=3:3
    ...
    rx_signal_buf = zeros(n_ant, num_sample_processing);
        file_id = fopen(['signals/user_rx_' num2str(st-1) '.dat'], 'rb');
        if (file_id < 0)
    ...
    for eigvec_idx = 1:n_ant
        ...
        for ii = 1:num_sample_processing-length(pn(1,:))
            signal_segment = rx_combined_signal(ii:ii+length(pn(3,:))-1);
            cross_correl_value1(ii) =pn(3,:)*signal_segment'/(norm(pn(3,:))*norm(signal_segment));
        end
